...
extern struct zproj_ctx *zproj_shell_get_ctx(void);
...
static int cmd_info(const struct shell *sh, size_t argc, char **argv)
{
	...
	struct zproj_ctx *ctx = zproj_shell_get_ctx();

	if (ctx == NULL || !ctx->initialized) {
		shell_error(sh, "zproj not initialized");
		return -1;
	}

	const struct zproj_model *m = ctx->model;
	...
	shell_print(sh, "Version:    %s", zproj_version_string());
...
static int cmd_facts(const struct shell *sh, size_t argc, char **argv)
{
	...
	for (uint16_t i = 0; i < ctx->model->fact_count; i++) {
		const struct zproj_fact_def *def = &ctx->model->facts[i];
		const struct zproj_fact_value *fv = &ctx->fact_values[i];
		const char *type_str;

		switch (def->type) {
		case ZPROJ_FACT_BOOL:
			type_str = "bool";
			break;
		case ZPROJ_FACT_INT32:
			type_str = "int32";
			break;
		case ZPROJ_FACT_UINT32:
			type_str = "uint32";
			break;
		case ZPROJ_FACT_ENUM:
			type_str = "enum";
			break;
		default:
			type_str = "?";
...
static int cmd_eval(const struct shell *sh, size_t argc, char **argv)
{
	...
	int ret = zproj_snapshot_begin(ctx, &snap);
	...
	ret = zproj_eval(ctx->model, &snap, &result, NULL);
	if (ret != ZPROJ_OK) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(zproj_cmds,
...
SHELL_CMD_REGISTER(zproj, &zproj_cmds, "zproj reasoning engine", NULL);
