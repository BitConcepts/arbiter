...
int ARBITER_watchdog_init(const struct device *wdt, uint32_t timeout_ms)
{
	...
	wdt_dev = wdt;
	LOG_INF("arbiter watchdog initialized (timeout=%u ms)", timeout_ms);

	return ARBITER_OK;
}

void ARBITER_watchdog_feed(void)
{
