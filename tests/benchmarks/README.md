...
Comparative benchmarks measuring the arbiter reasoning engine against
equivalent hand-coded C implementations.

## Available Benchmarks

| Benchmark | arbiter Model | Hand-coded Baseline | Key Metric |
|-----------|-------------|---------------------|------------|
...
- Both implementations run the **same algorithm** with the same inputs
  ...
  logic is expressed as hand-coded C or as a ARB model evaluated by
  the arbiter engine.

### RAM
- **Hand-coded**: `sizeof(struct hand_pid)` or `sizeof(struct hand_kf)` —
  just the bare state variables (24–28 bytes typical).
- **arbiter engine**: `sizeof(struct ARBITER_ctx)` — includes the fact value
...
- ROM cannot be measured at runtime.  After building, compare `.elf`
  ...
  ```sh
  # Build hand-coded-only version (strip arbiter from prj.conf)
  arm-none-eabi-size build/zephyr/zephyr.elf

  # vs. full arbiter engine version
  arm-none-eabi-size build/zephyr/zephyr.elf
  ```

- The delta includes:
  - arbiter engine code (`ARBITER_eval`, `ARBITER_snapshot_begin`, expression
...
- **Execution speed**: Expect 2–5× faster per tick.  The hand-coded PID
  is ~6 arithmetic operations.  The arbiter engine must iterate rule tables,
...
- **RAM**: Dramatically smaller.  A hand-coded PID is 24 bytes of state.
  The arbiter context carries per-fact metadata (timestamps, ranges),
...
- **ROM**: Smaller by several KB.  The hand-coded version compiles to
  a tight loop.  arbiter includes the generic engine, expression
...
- **Auditability for certification**: ARB models can be validated
  against a JSON schema, inspected via `arbiterc inspect`, and
  referenced in safety cases.  Hand-coded C requires separate
  requirement tracing.

- **Runtime inspection**: With `CONFIG_ARBITER_SHELL=y`, an operator
  can inspect live fact values, mode state, and rule firing from the
  Zephyr shell — impossible with a bare struct.

- **Separation of concerns**: Domain engineers author `.arb.yaml`
...
> arbiter is not faster or smaller than hand-coded C.  It is
...
2. Include a hand-coded baseline implementing the **same algorithm**
   as the arbiter model under test.
