...
"""Documentation emitter for ZRM models."""
...
def emit_docs(model: CanonicalModel) -> str:
    """Generate Markdown documentation for a canonical ZRM model."""
    lines = [
        f"# {model.name}",
        "",
        f"**ZRM Version:** {model.zrm_version}  ",
        f"**Model Hash:** `{model.model_hash[:16]}...`  ",
        f"**Compiler:** zprojc {model.compiler_version}",
        "",
    ]
