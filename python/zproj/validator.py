...
"""Semantic validator for ZRM models."""
...
def _validate_required_keys(data: dict[str, Any], diag: DiagnosticCollector) -> None:
    for key in ("zrm_version", "model", "target", "facts", "rules"):
        if key not in data:
            diag.error("ZPROJ-E-MISSING-KEY", f"(root).{key}", f"Required key '{key}' is missing")

    if "zrm_version" in data and data["zrm_version"] != 0.1:
        diag.error(
            "ZPROJ-E-VERSION",
            "(root).zrm_version",
            f"Unsupported ZRM version: {data['zrm_version']}",
        )

    if "target" in data:
        target = data["target"]
        if not isinstance(target, dict) or "rtos" not in target:
            diag.error("ZPROJ-E-TARGET", "(root).target", "target must have 'rtos' key")
...
def _validate_facts(data: dict[str, Any], diag: DiagnosticCollector) -> set[str]:
    ...
    if not isinstance(facts, list):
        diag.error("ZPROJ-E-FACTS-TYPE", "(root).facts", "facts must be a list")
        return fact_ids

    for i, fact in enumerate(facts):
        path = f"facts[{i}]"
        if not isinstance(fact, dict):
            diag.error("ZPROJ-E-FACT-TYPE", path, "Each fact must be a mapping")
            continue

        fid = fact.get("id")
        if not fid:
            diag.error("ZPROJ-E-FACT-NO-ID", path, "Fact missing 'id'")
            continue

        if fid in fact_ids:
            diag.error("ZPROJ-E-FACT-DUPLICATE", path, f"Duplicate fact id '{fid}'")
        fact_ids.add(fid)

        ftype = fact.get("type")
        if ftype not in VALID_FACT_TYPES:
            diag.error("ZPROJ-E-FACT-BAD-TYPE", f"{path}.type", f"Invalid fact type '{ftype}'")

        rng = fact.get("range")
        if rng is not None:
            if not isinstance(rng, list) or len(rng) != 2:
                diag.error("ZPROJ-E-FACT-BAD-RANGE", f"{path}.range", "range must be [min, max]")
            elif rng[0] > rng[1]:
                diag.error("ZPROJ-E-FACT-BAD-RANGE", f"{path}.range", "range min > max")
    ...
    for i, action in enumerate(actions):
        ...
        if aid:
            if aid in action_ids:
                diag.error("ZPROJ-E-ACTION-DUPLICATE", path, f"Duplicate action id '{aid}'")
            action_ids.add(aid)

        atype = action.get("type")
        if atype and atype not in VALID_ACTION_TYPES:
            diag.error("ZPROJ-E-ACTION-BAD-TYPE", f"{path}.type", f"Invalid action type '{atype}'")
...
def _validate_rules(
    ...
    for i, rule in enumerate(rules):
        ...
        if not isinstance(rule, dict):
            diag.error("ZPROJ-E-RULE-TYPE", path, "Each rule must be a mapping")
            continue

        rid = rule.get("id")
        if not rid:
            diag.error("ZPROJ-E-RULE-NO-ID", path, "Rule missing 'id'")
            continue

        if rid in rule_ids:
            diag.error("ZPROJ-E-RULE-DUPLICATE", path, f"Duplicate rule id '{rid}'")
        rule_ids.add(rid)

        rclass = rule.get("class")
        if rclass and rclass not in VALID_RULE_CLASSES:
            diag.error("ZPROJ-E-RULE-BAD-CLASS", f"{path}.class", f"Invalid class '{rclass}'")
...
def _validate_conditions(
    ...
    for group_key in ("all", "any"):
        ...
        if not isinstance(group, list):
            diag.error("ZPROJ-E-COND-TYPE", f"{path}.{group_key}", "Must be a list")
            continue
