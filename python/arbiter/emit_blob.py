...
_RULE_ELEM_SIZE = 22
_COND_ELEM_SIZE = 12  # fact_id(2) + op(1) + group(1) + value(4) + aux_value(4)
_EXPR_ELEM_SIZE = 20
...
def _pack_conditions(model: CanonicalModel) -> bytes:
    ...
    Wire layout per condition (12 bytes):
      ...
      group:       uint8
      value:       int32 LE
      aux_value:   int32 LE (for hysteresis falling threshold)
    """
    buf = bytearray()
    for c in model.conditions:
        ...
        buf += struct.pack("<HBBii", fact_id, op, group, int(val), int(aux_val))
    return bytes(buf)
